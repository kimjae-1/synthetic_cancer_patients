...
parser.add_argument("--data", default='../physio_data', dest="data")
...
dataset = pickle.load(open(options.data, "rb")) 
train_set=dataset["train_set"]
...
## no label or seq_len allowed

s_dim = sum([model.tgt_len for model in s_P.models[:-2]]) 
#age=1, sex=1, diag_code=1(12), result=1, seq_len=1 :: thus we also exclude two end 
#original = s_P.models[:5] :: exclude label, seq_len 
#Age    Gender  Height  ICUType     Weight  Label seq_len 1+1+1+4+1+1+1. len(ICUType.unique())=4
#model=labelbinarizer or minmaxscaler etc
print(s_dim) #original = 10-2, we=16-2
...
print(f'max seq_len in test set: {max(test_set["seq_len"])}')
...
for _ in range(4):
    ...
    if options.impute in ['zero', 'last']:
        ...
        #our :s_dim(static)=16?,  d_p.tgt_dim = 11 (num_dynamic_vars), +1?, miss_dim = 11.. maybe same with tgt_dim?
    else:
    ...
    best_auc = 0
    epochs = 40
    logger.info(f"training start with {len(train_batch)} batches of size 256")
    ...
    def evaluate(dev_batch):
        ...
        logger.info(f"auc:{auc}\tf1:{f1}\tacc:{acc}")
        return auc
    ...
    for i in range(epochs):
        ...
        for batch_x, batch_y in train_batch:
            ...
            bs, length, dim = dyn.size()
            target = sta[:, s_dim:s_dim+1].reshape(-1,1)
            sta = sta[:, :s_dim]
        ...
        logger.info(f"Epoch:{i+1} {time.time()-t1}\t{tot_loss/tot}")
